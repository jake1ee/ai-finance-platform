...
import { convertToMYRUnit } from "../utils/format-currency";
import { reportInsightPrompt } from "../validators/prompt";
...
export const generateReportService = async (
  ...
  return {
    period: periodLabel,
    summary: {
      income: convertToMYRUnit(totalIncome),
      expenses: convertToMYRUnit(totalExpenses),
      balance: convertToMYRUnit(availableBalance),
...
async function generateInsightsAI({
  ...
  try {
    const prompt = reportInsightPrompt({
      totalIncome: convertToMYRUnit(totalIncome),
      totalExpenses: convertToMYRUnit(totalExpenses),
      availableBalance: convertToMYRUnit(availableBalance),
      savingsRate: Number(savingsRate.toFixed(1)),
      categories,
      periodLabel,
    });
